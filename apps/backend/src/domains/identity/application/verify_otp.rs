...
impl VerifyOtpUseCase {
    pub async fn execute(&self, phone: &str, code: &str) -> Result<VerifyOtpOutput, AppError> {
        ...
        // 1. Look up the stored OTP session
        let entry = self
            .otp_repo
            .get(phone)
            .await?
        ...
        // 3. Delegate verification to the gateway
        //    For Infobip: calls POST /2fa/2/pin/{pinId}/verify
        //    For Mock:    checks session_token == code
        self.sms_gateway.verify_otp(&entry.session_token, code).await?;
