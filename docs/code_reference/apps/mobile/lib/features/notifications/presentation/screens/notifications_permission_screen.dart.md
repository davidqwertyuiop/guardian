# notifications_permission_screen.dart

* **File Path:** `apps/mobile/lib/features/notifications/presentation/screens/notifications_permission_screen.dart`
* **Type:** `DART`
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<AuthBloc, AuthState>(
      ...
      listener: (context, state) {
        if (state.step == AuthStep.notifications &&
            state.status == AuthStatus.success) {
          _showYouAreInSheet(context);
        }
      },
      child: OnboardingStepScreen(
