...
* **File Path:** `apps/mobile/lib/features/home/presentation/bloc/home_bloc.dart`
* **Type:** `DART`
...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  Future<void> _onLoadHomeData(
    ...
    final double initialLat = state.userLatitude != 0.0 ? state.userLatitude : 9.0578;
    final double initialLon = state.userLongitude != 0.0 ? state.userLongitude : 7.4951;
    final String initialGreeting = state.weatherGreeting.isNotEmpty ? state.weatherGreeting : "Lovely weather we're having today...";
    ...
    GpsService().getCurrentLocation().then((loc) {
      final double lat = loc['latitude'] ?? 9.0578;
      final double lon = loc['longitude'] ?? 7.4951;
      WeatherService.getWeatherGreeting(lat, lon).then((greeting) {
        if (!isClosed) {
          add(UpdateWeatherAndLocation(
            latitude: lat,
            longitude: lon,
