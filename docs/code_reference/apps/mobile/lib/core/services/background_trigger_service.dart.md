# background_trigger_service.dart

* **File Path:** `apps/mobile/lib/core/services/background_trigger_service.dart`
* **Type:** `DART`
...
class BackgroundTriggerService {
  static final BackgroundTriggerService _instance = BackgroundTriggerService._internal();
  ...
  void startListening() {
    ...
    debugPrint("BackgroundTriggerService: Listening for physical SOS tap triggers...");
    ...
    _accelerometerSubscription = userAccelerometerEventStream().listen((UserAccelerometerEvent event) {
      ...
      if (magnitude > _tapThreshold) {
        ...
        // Debounce checks
        if (_lastTapTime != null && now.difference(_lastTapTime!).inMilliseconds < _debounceMs) {
          return;
        }
  ...
  void _registerTap(DateTime time) async {
    ...
    _tapTimestamps.removeWhere((t) => time.difference(t).inSeconds > _timeWindowSeconds);
    
    debugPrint("BackgroundTriggerService: Tap registered. Count: ${_tapTimestamps.length}");
  ...
  void _triggerSos() async {
    ...
    debugPrint("BackgroundTriggerService: 5 taps detected! Launching SOS emergency trigger...");
    ...
    try {
      ...
      if (circles.isEmpty) {
        debugPrint("BackgroundTriggerService: User is not part of any circle. Cannot trigger SOS.");
        _isTriggering = false;
        return;
      }
      ...
      final result = await ApiService.triggerSos(
        ...
        latitude: latitude,
        longitude: longitude,
        address: "Background Gesture SOS Trigger",
      );

      debugPrint("BackgroundTriggerService: SOS triggered successfully: $result");
      ...
      debugPrint("BackgroundTriggerService: Error triggering background SOS: $e");
