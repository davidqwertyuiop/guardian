...
* **File Path:** `apps/backend/src/shared/middleware/auth.rs`
* **Type:** `RUST`
...
use crate::shared::{auth::jwt::{verify_access_token, Claims}, errors::AppError};
...
impl FromRequestParts<AppState> for AuthUser {
    ...
    async fn from_request_parts(
        ...
        if claims.token_type != "access" {
            return Err(AppError::Unauthorized("Token type must be 'access'".to_string()));
        }
