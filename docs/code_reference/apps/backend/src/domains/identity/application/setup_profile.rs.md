...
* **File Path:** `apps/backend/src/domains/identity/application/setup_profile.rs`
* **Type:** `RUST`
...
impl SetupProfileUseCase {
    pub async fn execute(&self, user_id: &str, name: &str) -> Result<User, AppError> {
        ...
        if name.len() > 100 {
            return Err(AppError::InvalidInput("Name must be 100 characters or fewer".to_string()));
        }
